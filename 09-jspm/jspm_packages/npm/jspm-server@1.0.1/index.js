...
(function(process) {
  ...
  LiveServer.start = function(options) {
    ...
    if (!options.proxyServer) {
      var app = connect().use(staticServer(root, html5mode)).use(connect.directory(root, {icons: true}));
      if (logLevel >= 2)
        app.use(connect.logger('dev'));
      server = http.createServer(app).listen(port, host);
    } else {
      var proxy = httpProxy.createProxyServer({target: options.proxyServer});
      server = connect().use(function(req, res, next) {
        ...
        proxy.web(req, res);
      }).listen(options.port || 8080).on('error', console.log.bind(console));
    }
    ...
    chokidar.watch(root, {
      ...
      if (event == 'error') {
        ...
        if (logLevel >= 1)
          console.log(("Change detected: " + relativePath).cyan);
